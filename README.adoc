...
After running `ng build --prod`, you can start Quarkus with `mvn compile quarkus:dev` and load the Angular app with http://localhost:8080
...
docker run -i --rm -p 8080:8080 quarkus/portfolio
----
